...
2. Run `test/run.sh` and shellcheck; commit. The commit message is the
   changelog, so write it like the ones before it.
3. Tag it and push the tag: `git tag -a v<version> -m v<version> && git push
